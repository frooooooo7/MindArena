...
export function startRoomCleanup(io: Server): void {
  ...
  cleanupInterval = setInterval(() => {
    ...
    if (removed > 0) {
      console.log(
        `[ROOM] Cleanup: removed ${removed} stale rooms, ${gameRooms.size} remaining`,
      );
...
export function updatePlayer(
  ...
  playerId: string,
  data: Partial<
    Pick<GamePlayer, "currentIndex" | "hasFailed" | "currentLevel">
  >,
): GamePlayer | undefined {
  const room = gameRooms.get(roomId);
  if (!room) return undefined;
  ...
  try {
    ...
    return Array.from(gameRooms.values())
      ...
      .map((room) => ({
        ...
        gameType:
          (room.gameType || "unknown").charAt(0).toUpperCase() +
          room.gameType.slice(1).toLowerCase(),
        status: (room.status || "waiting") as
          | "waiting"
          | "playing"
          | "finished",
