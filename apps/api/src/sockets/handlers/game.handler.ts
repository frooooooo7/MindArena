 ...
 */
function startRoundTimer(io: Server, roomId: string, gameType: string) {
...
function handleRoundTimeout(io: Server, roomId: string, gameType: string) {
  ...
  // If both completed or neither completed, it's a tie - pick random loser
  // If only one completed, the other loses
  let loserId: string;
  let winnerId: string;

  if (player1Complete && !player2Complete) {
    winnerId = room.players[0].id;
    loserId = room.players[1].id;
  } else if (!player1Complete && player2Complete) {
    winnerId = room.players[1].id;
    loserId = room.players[0].id;
  } else if (!player1Complete && !player2Complete) {
    // Neither completed - loser is whoever has less progress
    const p1Progress =
      gameType === "chimp"
        ? chimpMemory.getPlayerProgress(roomId, room.players[0].id)
            ?.completedCount || 0
        : room.players[0].currentIndex;
    const p2Progress =
      gameType === "chimp"
        ? chimpMemory.getPlayerProgress(roomId, room.players[1].id)
            ?.completedCount || 0
    ...
    // Both completed - shouldn't happen, but advance to next level
    return;
  }
...
function startGameCountdown(io: Server, roomId: string) {
  ...
  setTimeout(() => {
    ...
    if (gameType === "chimp") {
      ...
      setTimeout(() => {
        startRoundTimer(io, roomId, gameType);
      }, 2000);
    } else {
      ...
      // Sequence animation takes ~(sequence.length * 1000ms)
