...
import { ARENA_EVENTS, QueueCountPayload } from "@mindarena/shared";
...
export function ArenaModes({ onJoin }: ArenaModesProps) {
  ...
  useEffect(() => {
    ...
    socket.on(ARENA_EVENTS.QUEUE_COUNT_UPDATE, handleQueueCountUpdate);
  ...
  const modes = [
    ...
    },
    {
      ...
      description:
        "Create a custom room and invite your friends for a private cognitive battle.",
      icon: Users,
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      {modes.map((mode, i) => (
        <div
          key={i}
          className="group relative p-8 rounded-[2rem] border border-border/40 bg-card/50 overflow-hidden hover:border-violet-500/40 transition-colors duration-200 flex flex-col h-full shadow-lg"
        >
          {/* Color splash - simplified */}
          <div
            className={`absolute top-0 right-0 -translate-y-1/2 translate-x-1/2 h-32 w-32 bg-gradient-to-br ${mode.color} opacity-10 rounded-full pointer-events-none`}
          />

          <div className="relative mb-6">
            <div
              className={`inline-flex p-3 rounded-2xl bg-gradient-to-br ${mode.color} text-white shadow-lg`}
            >
